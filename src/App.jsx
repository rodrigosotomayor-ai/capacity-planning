...
export default function App() {
  ...
  const load = useCallback(async () => {
    setPhase("loading");
    try {
      const [teams,people,projects,months,avail,allocs,absences] = await Promise.all([
        db("cap_teams").select(),
      ...
      setData({teams,people,projects,months,avail,allocs,absences});
  ...
  function md() {
    ...
    const people=teamPeople.map(p=>{
      ...
      return{...p, avail_h: Math.max(0,baseH-absH), base_h: baseH, abs_h: absH, avail_id: avRec?.id||null};
    });
  ...
  return(
    <div style={{display:"flex",height:"calc(100vh - 32px)",gap:12,position:"relative"}}>
      ...
      {modal?.type==="editAbsence"&&cur&&(
        <Modal title={modal.absence.id?"Editar ausencia":"Registrar ausencia"} onClose={()=>setModal(null)}>
          <AbsenceForm
...
function AbsenceForm({ initial, people, onSave, onCancel }) {
  ...
  const TYPE_OPTIONS = [
    ...
    {v:"feriado",       l:"📅 Feriado",         desc:"Feriado legal o festivo"},
  ];

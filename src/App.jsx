            ...
            <div style={{...glass,padding:20}}><div style={{fontSize:10,color:T.d,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",marginBottom:12}}>Proyectos en este mes</div>
              ...
              {cur.projects.map((pr,pi)=>{
                ...
                return(<div key={pr.id} style={{padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
                  ...
                  {budgetPct!==null&&(<div style={{marginTop:5,paddingLeft:17}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}>
                      <span style={{fontSize:9,color:T.d}}>Presupuesto</span>
                      <span style={{fontSize:9,fontFamily:"monospace",color:budgetPct>100?"#FF4757":budgetPct>85?"#FFA502":"#2ED573",fontWeight:700}}>{fmt(tot)}h / {pr.budget_hours}h ({budgetPct}%)</span>
                    </div>
                    <div style={{height:3,background:"rgba(255,255,255,0.06)",borderRadius:2,overflow:"hidden"}}>
                      <div style={{height:"100%",width:`${Math.min(budgetPct,100)}%`,background:budgetPct>100?"#FF4757":budgetPct>85?"#FFA502":"#2ED573",borderRadius:2}}/>
                    </div>
                  </div>)}
                </div>);
              })}
        ...
        {view==="proyectos"&&cur&&(()=>{
          ...
          return(<div style={glass}><div style={{padding:"12px 18px",borderBottom:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",justifyContent:"space-between"}}><span style={{fontSize:11,color:T.d,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em"}}>Proyectos · {cur.month.label}</span><button onClick={()=>setModal({type:"editProject",project:{name:"",type:"proyecto"}})} style={{...BP,fontSize:11,padding:"6px 12px"}}><i className="ti ti-plus"/>Nuevo proyecto</button></div>{projects.length===0&&<div style={{padding:28,textAlign:"center",color:T.d,fontSize:13}}>No hay proyectos activos para el equipo {team}.</div>}{projects.map((p,pi)=>{const tot=people.reduce((s,pe)=>s+getH(pe.id,p.id),0);const assigned=people.filter(pe=>getH(pe.id,p.id)>0);const tC={fee:[A,"rgba(57,18,250,0.12)"],proyecto:["#2ED573","rgba(46,213,115,0.1)"],interno:[T.d,"rgba(255,255,255,0.04)"]}[p.type];const today2=new Date();today2.setHours(0,0,0,0);
                  ...
                  return(<div key={p.id} style={{borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
                    ...
                    {(pctTime!==null||budgetPct!==null)&&(<div style={{display:"flex",gap:16,padding:"0 18px 10px 38px"}}>
                      {pctTime!==null&&(<div style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:9,color:T.d}}>Tiempo transcurrido</span><span style={{fontSize:9,color:isOverdue?"#FF4757":pctTime>85?"#FFA502":T.d,fontWeight:700}}>{pctTime}%</span></div><div style={{height:3,background:"rgba(255,255,255,0.06)",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:`${pctTime}%`,background:isOverdue?"#FF4757":pctTime>85?"#FFA502":A,borderRadius:2}}/></div></div>)}
                      {budgetPct!==null&&(<div style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:9,color:T.d}}>Presupuesto HH</span><span style={{fontSize:9,color:budgetPct>100?"#FF4757":budgetPct>85?"#FFA502":"#2ED573",fontWeight:700}}>{fmt(tot)}h / {p.budget_hours}h ({budgetPct}%)</span></div><div style={{height:3,background:"rgba(255,255,255,0.06)",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:`${Math.min(budgetPct,100)}%`,background:budgetPct>100?"#FF4757":budgetPct>85?"#FFA502":"#2ED573",borderRadius:2}}/></div></div>)}
                    </div>)}
                  </div>);})}</div>);
        })()}

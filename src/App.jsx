...
export default function App(){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const [teams,people,projects,months,avail,allocs,absences,holidays]=await Promise.all([
        ...
        db("cap_absences").select(),db("cap_holidays").select(),
      ]);
  ...
  async function createMonth(){
    ...
    if(nm.copyFrom){const prevAv=data.avail.filter(a=>a.month_id===nm.copyFrom);for(const a of prevAv){const x=await db("cap_availability").insert({month_id:newId,person_id:a.person_id,available_hours:a.available_hours});if(x[0])newAv.push(x[0]);}const feeIds=data.projects.filter(p=>p.team_id===teamObj.id&&p.type==="fee").map(p=>p.id);const fees=data.allocs.filter(a=>a.month_id===nm.copyFrom&&feeIds.includes(a.project_id));for(const a of fees){const x=await db("cap_allocations").insert({month_id:newId,person_id:a.person_id,project_id:a.project_id,hours:a.hours});if(x[0])newAl.push(x[0]);}}else{const tp=data.people.filter(p=>p.team_id===teamObj.id&&p.is_active);for(const p of tp){const x=await db("cap_availability").insert({month_id:newId,person_id:p.id,available_hours:p.default_hours});if(x[0])newAv.push(x[0]);}}
    setData(d=>({...d,months:[...d.months,r[0]],avail:[...d.avail,...newAv],allocs:[...d.allocs,...newAl]}));setMonthId(newId);setView("matrix");setSub("grid");notify(`✓ ${label} creado`);}finally{setSaving(false);}
  }
      ...
      <div style={{flex:1,overflowY:"auto",paddingRight:2}}>
        ...
        {view==="matrix"&&sub==="config"&&cur&&(()=>{
          ...
          return(<div style={{maxWidth:720}}>
            ...
            <div style={{...glass,padding:20}}><div style={{fontSize:10,color:T.d,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",marginBottom:12}}>Proyectos activos en este mes</div>{cur.projects.map((pr,pi)=>{const tot=people.reduce((s,p)=>s+cur.getH(p.id,pr.id),0);const tC={fee:A,proyecto:"#2ED573",interno:T.d}[pr.type];return(<div key={pr.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}><span style={{width:7,height:7,borderRadius:"50%",background:PC[pi%PC.length],flexShrink:0}}/><span style={{flex:1,fontSize:12,color:"#fff",fontWeight:500}}>{pr.name}</span><span style={{fontSize:10,padding:"2px 8px",borderRadius:20,background:"rgba(255,255,255,0.07)",color:tC,fontWeight:600}}>{TYPE_LABELS[pr.type]}</span><span style={{fontFamily:"monospace",fontSize:11,color:tot>0?"#fff":T.d,fontWeight:700}}>{tot>0?fmt(tot)+"h":"sin horas"}</span></div>);})}<button onClick={()=>setModal({type:"editProject",project:{name:"",type:"proyecto"}})} style={{...BG,marginTop:14,fontSize:11}}><i className="ti ti-plus"/>Nuevo proyecto</button></div>
          </div>);
        })()}

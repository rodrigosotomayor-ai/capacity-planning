...
function db(table){
  ...
  return{
    ...
    update:(f,d)=>fetch(`${base}?${f}`,{method:"PATCH",headers:{...h,Prefer:"return=representation"},body:JSON.stringify(d)}).then(r=>r.json()),
...
function AbsenceForm({initial,people,holidays,existingAbsences,monthYear,onSave,onCancel}){
  ...
  const canSave=personId&&totalHours>0;
...
function PersonForm({initial,onSave,onCancel}){
  const [f,setF]=useState(initial);const set=(k,v)=>setF(x=>({...x,[k]:v}));
  return(<div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}><div style={{gridColumn:"1/-1"}}><label style={{display:"block",fontSize:11,color:T.m,marginBottom:5,fontWeight:600}}>Nombre completo *</label><input value={f.name} onChange={e=>set("name",e.target.value)} placeholder="Ej: María González" style={{...INP,boxSizing:"border-box"}}/></div><div><label style={{display:"block",fontSize:11,color:T.m,marginBottom:5,fontWeight:600}}>Cargo / Rol</label><input value={f.role||""} onChange={e=>set("role",e.target.value)} placeholder="Ej: UX Designer" style={{...INP,boxSizing:"border-box"}}/></div><div><label style={{display:"block",fontSize:11,color:T.m,marginBottom:5,fontWeight:600}}>Iniciales</label><input value={f.initials||""} onChange={e=>set("initials",e.target.value)} placeholder="Ej: MG" maxLength={3} style={{...INP,boxSizing:"border-box"}}/></div><div><label style={{display:"block",fontSize:11,color:T.m,marginBottom:5,fontWeight:600}}>HH disponibles / mes</label><input value={f.default_hours||"160"} onChange={e=>set("default_hours",e.target.value)} type="number" style={{...INP,boxSizing:"border-box"}}/></div></div><div style={{fontSize:11,color:T.d,marginBottom:16}}>Las horas por mes son el valor por defecto. Puedes ajustarlas mes a mes en Configurar mes.</div><div style={{display:"flex",gap:8,justifyContent:"flex-end"}}><button onClick={onCancel} style={BG}>Cancelar</button><button onClick={()=>onSave(f)} disabled={!f.name} style={{...BP,opacity:f.name?1:0.5}}><i className={`ti ti-${f.id?"check":"user-plus"}`}/>{f.id?"Guardar cambios":"Agregar persona"}</button></div></div>);
}
...
function AnualView({ data, team }) {
  ...
  const monthMetrics = filteredMonths.map(m=>{
    ...
    const monthPeople = avails.map(a=>{
      ...
      const absH=absences.filter(x=>x.person_id===a.person_id).reduce((s,x)=>s+parseFloat(x.hours),0);
      const availH=Math.max(0,a.available_hours-absH-holH);
    ...
    return{...m,monthPeople,totalAvail,totalSold,occPct,overCount,absHTotal,byType,holH};
  });
  ...
  const personMetrics = allPeople.map(p=>{
    const byMonth = filteredMonths.map(m=>{
      ...
      const absH=(data.absences||[]).filter(a=>a.month_id===m.id&&a.person_id===p.id).reduce((s,a)=>s+parseFloat(a.hours),0);
      const holH=(data.holidays||[]).filter(h=>h.year===m.year&&h.month_num===m.month_num).reduce((s,h)=>s+parseFloat(h.hours),0);
    ...
    const totalS  = active.reduce((s,m)=>s+m.soldH,0);
    const totalA  = active.reduce((s,m)=>s+m.availH,0);
    return{...p,byMonth,avgPct,totalSold:totalS,totalAvail:totalA,activeMonths:active.length};
  }).filter(p=>p.activeMonths>0).sort((a,b)=>b.avgPct-a.avgPct);

  // ── Annual KPIs ─────────────────────────────────────────────────────────────
  const annualSold  = fmt(monthMetrics.reduce((s,m)=>s+m.totalSold,0));
  const annualAvail = monthMetrics.reduce((s,m)=>s+m.totalAvail,0);
  const annualOcc   = annualAvail>0?Math.round(monthMetrics.reduce((s,m)=>s+m.totalSold,0)/annualAvail*100):0;
  const peakMonth   = [...monthMetrics].sort((a,b)=>b.occPct-a.occPct)[0];
  const annualAbs   = fmt(monthMetrics.reduce((s,m)=>s+m.absHTotal,0));
  ...
  function exportXLS() {
    ...
    const sheet1Data = [
      ["Mes","Año","HH Disponibles","HH Asignadas","% Ocupación","Sobreocupados","HH Ausencias","HH Feriados","HH Proyectos","HH Fees","HH Interno"],
      ...monthMetrics.map(m=>[
        MN[m.month_num-1], m.year,
        m.totalAvail, fmt(m.totalSold), m.occPct+"%",
        m.overCount, fmt(m.absHTotal), fmt(m.holH),
        fmt(m.byType.proyecto), fmt(m.byType.fee), fmt(m.byType.interno)
      ])
    ];
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(sheet1Data), "Resumen mensual");

    // Sheet 2: Ocupación por persona
    const headers2 = ["Persona","Cargo","Meses activos","HH Asignadas","HH Disponibles","% Ocupación promedio",
      ...filteredMonths.map(m=>MN[m.month_num-1]+" (%)"), ...filteredMonths.map(m=>MN[m.month_num-1]+" (HH)")];
    const rows2 = personMetrics.map(p=>[
      p.name, p.role||"—", p.activeMonths,
      fmt(p.totalSold), fmt(p.totalAvail), p.avgPct+"%",
      ...filteredMonths.map((_,i)=>{ const bm=p.byMonth[i]; return bm?Math.round(bm.pct*100)+"%":"—"; }),
      ...filteredMonths.map((_,i)=>{ const bm=p.byMonth[i]; return bm?fmt(bm.soldH):"—"; }),
    ]);
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([headers2,...rows2]), "Ocupación personas");

    // Sheet 3: HH por proyecto
    const projRows = [];
    const allProjIds = [...new Set((data.allocs||[]).filter(a=>filteredMonths.some(m=>m.id===a.month_id)).map(a=>a.project_id))];
    projRows.push(["Proyecto","Tipo","Total HH",...filteredMonths.map(m=>MN[m.month_num-1])]);
    allProjIds.forEach(pid=>{
      const proj=data.projects.find(p=>p.id===pid); if(!proj||proj.team_id!==teamObj?.id) return;
      const monthHH=filteredMonths.map(m=>{const allocs=(data.allocs||[]).filter(a=>a.month_id===m.id&&a.project_id===pid);return allocs.length?fmt(allocs.reduce((s,a)=>s+parseFloat(a.hours),0)):0;});
      const total=monthHH.reduce((s,h)=>s+h,0); if(!total) return;
      projRows.push([proj.name,TYPE_LABELS[proj.type]||proj.type,fmt(total),...monthHH]);
    });
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(projRows), "HH por proyecto");

    // Sheet 4: Ausencias
    const absRows = [["Persona","Cargo","Mes","Tipo","HH","Notas"]];
    filteredMonths.forEach(m=>{
      const mAbs=(data.absences||[]).filter(a=>a.month_id===m.id);
      mAbs.forEach(a=>{
        const p=data.people.find(x=>x.id===a.person_id);
        absRows.push([p?.name||"—",p?.role||"—",m.label,ABS_LABEL[a.type]||a.type,a.hours,a.notes||""]);
      });
    });
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(absRows), "Ausencias");
  ...
  function StackedBars({ metrics }) {
    ...
    const W=560,H=160,padL=28,padB=24,padT=10,barW=Math.min(40,(W-padL)/metrics.length-4);
    const maxH=Math.max(...metrics.map(m=>m.totalSold),1);
    const scale=v=>(H-padB-padT)*v/maxH;
    const COLORS={fee:"#2ED573",proyecto:A,interno:"rgba(255,255,255,0.2)"};
    return(
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet">
        {/* Y grid lines */}
        {[0.25,0.5,0.75,1].map(f=>{
          const y=H-padB-(H-padB-padT)*f;
          return(<g key={f}><line x1={padL} y1={y} x2={W} y2={y} stroke="rgba(255,255,255,0.06)" strokeWidth={1}/><text x={padL-4} y={y+4} textAnchor="end" fill="rgba(255,255,255,0.3)" fontSize={8} fontFamily="Montserrat">{Math.round(maxH*f)}</text></g>);
        })}
        {/* Bars */}
        {metrics.map((m,i)=>{
          const x=padL+(W-padL)/metrics.length*i+(W-padL)/metrics.length/2-barW/2;
          let y=H-padB;
          return(<g key={m.id}>
            {["interno","fee","proyecto"].map(type=>{
              const h=scale(m.byType[type]||0); if(!h) return null;
              y-=h;
              return(<rect key={type} x={x} y={y} width={barW} height={h} rx={type==="interno"?0:0} fill={COLORS[type]} opacity={0.85}/>);
            })}
            <text x={x+barW/2} y={H-padB+12} textAnchor="middle" fill="rgba(255,255,255,0.5)" fontSize={9} fontFamily="Montserrat">{MN[m.month_num-1].substring(0,3)}</text>
            <text x={x+barW/2} y={H-padB-scale(m.totalSold)-4} textAnchor="middle" fill="rgba(255,255,255,0.6)" fontSize={8} fontFamily="Montserrat">{m.occPct}%</text>
          </g>);
        })}
      </svg>
    );
  ...
  function OccLine({ metrics }) {
    ...
    const W=560,H=130,padL=28,padB=24,padT=10;
    const sx=i=>padL+(W-padL)/(metrics.length-1)*i;
    const sy=v=>H-padB-(H-padB-padT)*Math.min(v,120)/120;
    const d=metrics.map((m,i)=>`${i===0?"M":"L"} ${sx(i)} ${sy(m.occPct)}`).join(" ");
    return(
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet">
        <defs><linearGradient id="og" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={A} stopOpacity={0.25}/><stop offset="100%" stopColor={A} stopOpacity={0}/></linearGradient></defs>
        {/* Reference lines */}
        {[0,50,85,100].map(v=>{
          const y=sy(v);
          return(<g key={v}><line x1={padL} y1={y} x2={W} y2={y} stroke={v===85?"rgba(255,165,2,0.25)":v===100?"rgba(255,71,87,0.25)":"rgba(255,255,255,0.06)"} strokeWidth={v===85||v===100?1.5:1} strokeDasharray={v===85||v===100?"4,3":"none"}/><text x={padL-4} y={y+3} textAnchor="end" fill="rgba(255,255,255,0.3)" fontSize={8} fontFamily="Montserrat">{v}%</text></g>);
        })}
        <path d={`${d} L ${sx(metrics.length-1)} ${H-padB} L ${sx(0)} ${H-padB} Z`} fill="url(#og)"/>
        <path d={d} fill="none" stroke={A} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"/>
        {metrics.map((m,i)=>{
          const col=oc(m.occPct/100);
          return(<g key={m.id}><circle cx={sx(i)} cy={sy(m.occPct)} r={4} fill={col} stroke="rgba(7,7,20,0.8)" strokeWidth={1.5}/><text x={sx(i)} y={H-padB+12} textAnchor="middle" fill="rgba(255,255,255,0.45)" fontSize={9} fontFamily="Montserrat">{MN[m.month_num-1].substring(0,3)}</text></g>);
        })}
      </svg>
    );
  }
  ...
  function Heatmap({ people, months }) {
    ...
    const cellW=Math.max(32,Math.min(56,480/months.length));
    const cellH=28,labelW=130,headerH=28;
    const W=labelW+cellW*months.length+16;
    const H=headerH+cellH*people.length+8;
    return(
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMinYMin meet" style={{overflow:"visible"}}>
        {/* Month headers */}
        {months.map((m,i)=><text key={m.id} x={labelW+cellW*i+cellW/2} y={headerH-6} textAnchor="middle" fill="rgba(255,255,255,0.45)" fontSize={9} fontFamily="Montserrat" fontWeight={600}>{MN[m.month_num-1].substring(0,3)}</text>)}
        {/* Rows */}
        {people.map((p,ri)=>{
          const y=headerH+cellH*ri;
          return(<g key={p.id}>
            <text x={labelW-8} y={y+cellH/2+4} textAnchor="end" fill="rgba(255,255,255,0.65)" fontSize={10} fontFamily="Montserrat">{p.name.split(" ")[0]}{p.name.split(" ").length>1?" "+p.name.split(" ")[1]?.charAt(0)+".":""}</text>
            {months.map((m,ci)=>{
              const bm=p.byMonth[ci];
              if(!bm) return(<rect key={m.id} x={labelW+cellW*ci+1} y={y+1} width={cellW-2} height={cellH-2} rx={3} fill="rgba(255,255,255,0.03)"/>);
              const pctV=Math.round(bm.pct*100);
              const col=oc(bm.pct);
              const bgCol=ob(bm.pct);
              return(<g key={m.id}>
                <rect x={labelW+cellW*ci+1} y={y+1} width={cellW-2} height={cellH-2} rx={3} fill={bgCol}/>
                <text x={labelW+cellW*ci+cellW/2} y={y+cellH/2+4} textAnchor="middle" fill={col} fontSize={9} fontFamily="Montserrat" fontWeight={700}>{pctV}%</text>
              </g>);
            })}
          </g>);
        })}
  ...
  if(!filteredMonths.length) return(
    <div style={{...glassHi,padding:32,textAlign:"center",maxWidth:500,margin:"40px auto"}}>
      ...
      <div style={{fontSize:13,color:T.m}}>No hay meses registrados para el equipo {team} en {year}. Crea meses primero.</div>
    </div>
  );
    ...
    <div>
      ...
      <div style={{...glass,padding:"12px 18px",marginBottom:12,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <select value={fromMonth} onChange={e=>setFromMonth(parseInt(e.target.value))} style={{...SEL,width:130,padding:"5px 8px",fontSize:11}}>
            {MN.map((m,i)=><option key={i} value={i+1}>{m}</option>)}
          </select>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:11,color:T.d,fontWeight:600}}>Hasta</span>
          <select value={toMonth} onChange={e=>setToMonth(parseInt(e.target.value))} style={{...SEL,width:130,padding:"5px 8px",fontSize:11}}>
            {MN.map((m,i)=><option key={i} value={i+1}>{m}</option>)}
          </select>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10,marginBottom:12}}>
        {[
          {v:annualSold+"h",  l:"HH Asignadas",    n:`de ${annualAvail} disponibles`,          c:"#fff"},
          {v:`${annualOcc}%`, l:"Ocupación anual",  n:"promedio del período",                   c:oc(annualOcc/100)},
          {v:peakMonth?MN[peakMonth.month_num-1]:"—", l:"Mes más cargado", n:peakMonth?peakMonth.occPct+"% ocupación":"", c:"#FFA502"},
          {v:annualAbs+"h",   l:"HH Ausencias",     n:"vacaciones + licencias",                 c:"rgba(255,255,255,0.6)"},
          {v:`${feeRatio}%`,  l:"Ratio fee",        n:`${100-feeRatio}% proyectos`,             c:"#2ED573"},
        ].map((k,i)=>(
          <div key={i} style={{...glass,padding:"14px 16px"}}>
            <div style={{fontSize:10,color:T.d,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",marginBottom:6}}>{k.l}</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
        ...
        <div style={{...glass,padding:20}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div style={{fontSize:10,color:T.d,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em"}}>Ocupación % · mensual</div>
            <div style={{display:"flex",gap:10}}>{[["#FFA502","85% umbral"],["#FF4757","100% límite"]].map(([c,l])=>(<div key={l} style={{display:"flex",alignItems:"center",gap:4,fontSize:10,color:T.m}}><div style={{width:14,height:2,background:c,borderRadius:1}}/>{l}</div>))}</div>
          </div>
      ...
      <div style={{...glass,padding:20,marginBottom:10}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          ...
          <div style={{display:"flex",gap:10}}>{[["#2ED573","Óptimo 50–84%"],["#FFA502","Alto 85–100%"],["#FF4757","Sobrecarga >100%"],["rgba(255,255,255,0.2)","Sub-utilizado <50%"]].map(([c,l])=>(<div key={l} style={{display:"flex",alignItems:"center",gap:4,fontSize:10,color:T.m}}><div style={{width:8,height:8,borderRadius:2,background:c}}/>{l}</div>))}</div>
        </div>
        <Heatmap people={personMetrics} months={filteredMonths}/>
      </div>

      {/* ── People ranking + Top projects ── */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        <div style={{...glass,padding:20}}>
          <div style={{fontSize:10,color:T.d,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",marginBottom:14}}>Ranking ocupación · personas</div>
          {personMetrics.slice(0,10).map((p,i)=>{
            const col=oc(p.avgPct/100);
            return(<div key={p.id} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
              <span style={{fontSize:11,color:T.d,fontWeight:700,minWidth:16,textAlign:"right"}}>{i+1}</span>
              <div style={{width:28,height:28,borderRadius:"50%",background:ob(p.avgPct/100),border:`1px solid ${col}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:col,flexShrink:0}}>{p.initials||ini(p.name)}</div>
              <div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:600,color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.name}</div><div style={{fontSize:10,color:A}}>{p.role||"—"}</div></div>
              <div style={{width:80,height:4,background:"rgba(255,255,255,0.06)",borderRadius:2,overflow:"hidden",flexShrink:0}}><div style={{height:"100%",width:`${Math.min(p.avgPct,100)}%`,background:col,borderRadius:2}}/></div>
              <span style={{fontSize:11,color:col,fontWeight:700,minWidth:38,textAlign:"right"}}>{p.avgPct}%</span>
            </div>);
          })}
        </div>
        <div style={{...glass,padding:20}}>
          <div style={{fontSize:10,color:T.d,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",marginBottom:14}}>Top proyectos · HH acumuladas</div>
          {(()=>{
            const projTotals={};
            filteredMonths.forEach(m=>{
              (data.allocs||[]).filter(a=>a.month_id===m.id).forEach(a=>{projTotals[a.project_id]=(projTotals[a.project_id]||0)+parseFloat(a.hours);});
            });
            const sorted=Object.entries(projTotals).map(([id,h])=>({...data.projects.find(p=>p.id===id),hours:h})).filter(p=>p.id&&p.team_id===teamObj?.id).sort((a,b)=>b.hours-a.hours).slice(0,10);
            const maxH=sorted[0]?.hours||1;
            return sorted.map((p,i)=>{
              const tC={fee:"#2ED573",proyecto:A,interno:T.d}[p.type];
              return(<div key={p.id} style={{marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                  <span style={{fontSize:11,color:T.m,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1,marginRight:8}}>{p.name}</span>
                  <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
                    <span style={{fontSize:10,padding:"1px 7px",borderRadius:10,background:"rgba(255,255,255,0.06)",color:tC,fontWeight:600}}>{TYPE_LABELS[p.type]||p.type}</span>
                    <span style={{fontSize:11,fontWeight:700,color:"#fff",minWidth:44,textAlign:"right"}}>{fmt(p.hours)}h</span>
                  </div>
                </div>
...
function AusenciasView({cur,monthId,data,setModal,deleteAbsence,deleteHoliday}){
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:12}}>
      {[{v:people.filter(p=>(data.absences||[]).some(a=>a.month_id===monthId&&a.person_id===p.id)).length,l:"Personas con ausencia",n:month.label,c:"#fff"},{v:fmt(totalAbsH)+"h",l:"HH ausencias individuales",n:"vacaciones + licencias",c:"#FFA502"},{v:monthHolidays.length,l:"Feriados del mes",n:"aplican a todo el equipo",c:"#7B6FFA"},{v:fmt(totalHolH)+"h",l:"HH feriados",n:"descontadas a todos",c:"#7B6FFA"}].map((k,i)=>(
    ...
    {tab==="feriados"&&(<div style={glass}>
      <div style={{padding:"12px 18px",borderBottom:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div><span style={{fontSize:11,color:T.d,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em"}}>Feriados {month.year}</span><span style={{fontSize:11,color:T.d,marginLeft:10}}>— aplican automáticamente a todo el equipo</span></div>
...
export default function App(){
  ...
  function md(){
    ...
    const people=teamPeople.map(p=>{
      ...
      const absH=(data.absences||[]).filter(a=>a.month_id===monthId&&a.person_id===p.id).reduce((s,a)=>s+parseFloat(a.hours),0);
      const holH=(data.holidays||[]).filter(h=>h.year===month.year&&h.month_num===month.month_num).reduce((s,h)=>s+parseFloat(h.hours),0);
  ...
  async function upsertAlloc(mid,pid,projId,hours){
    ...
    let newAv=null;const hasAv=data.avail.find(a=>a.month_id===mid&&a.person_id===pid);
    if(!hasAv){const p=data.people.find(x=>x.id===pid);const r=await db("cap_availability").insert({month_id:mid,person_id:pid,available_hours:p?.default_hours||160});if(r[0])newAv=r[0];}
    setData(d=>{const newAvail=newAv?[...d.avail,newAv]:d.avail;const rest=d.allocs.filter(a=>!(a.month_id===mid&&a.person_id===pid&&a.project_id===projId));if(h>0)rest.push({id:"_tmp",month_id:mid,person_id:pid,project_id:projId,hours:h});return{...d,avail:newAvail,allocs:rest};});
    try{if(h>0){const r=await db("cap_allocations").upsert({month_id:mid,person_id:pid,project_id:projId,hours:h,updated_at:new Date().toISOString()});if(r[0])setData(d=>({...d,allocs:d.allocs.map(a=>a.id==="_tmp"?r[0]:a)}))}else{await db("cap_allocations").remove(`month_id=eq.${mid}&person_id=eq.${pid}&project_id=eq.${projId}`);setData(d=>({...d,allocs:d.allocs.filter(a=>!(a.month_id===mid&&a.person_id===pid&&a.project_id===projId))}));}}finally{setSaving(false);}
  }
...
if(monthId){const mp=await db("cap_month_projects").insert({month_id:monthId,project_id:r[0].id});if(mp[0])setData(d=>({...d,monthProjects:[...d.monthProjects,mp[0]]}))};}notify("✓ Proyecto creado y añadido al mes actual");}setModal(null);}finally{setSaving(false);}}
  ...
  async function savePerson(person){setSaving(true);try{const teamId=data.teams.find(t=>t.slug===team)?.id;if(person.id){const r=await db("cap_people").update(`id=eq.${person.id}`,{name:person.name,role:person.role,initials:person.initials||ini(person.name),default_hours:parseInt(person.default_hours)||160});if(r[0])setData(d=>({...d,people:d.people.map(p=>p.id===person.id?r[0]:p)}));notify("✓ Persona actualizada");}else{const r=await db("cap_people").insert({name:person.name,role:person.role,initials:person.initials||ini(person.name),team_id:teamId,default_hours:parseInt(person.default_hours)||160,is_active:true});if(r[0]){setData(d=>({...d,people:[...d.people,r[0]]}));if(monthId){const av=await db("cap_availability").insert({month_id:monthId,person_id:r[0].id,available_hours:parseInt(person.default_hours)||160});if(av[0])setData(d=>({...d,avail:[...d.avail,av[0]]}));}notify("✓ Persona agregada");};}setModal(null);}finally{setSaving(false);}}

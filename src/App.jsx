    ...
    try{
      ...
      if(person.id){
        ...
        if(newHours!==oldDefault){
          const toUpdate=data.avail.filter(a=>a.person_id===person.id&&a.available_hours===oldDefault);
          for(const a of toUpdate){
            await db("cap_availability").update(`id=eq.${a.id}`,{available_hours:newHours});
          }
          setData(d=>({...d,avail:d.avail.map(a=>a.person_id===person.id&&a.available_hours===oldDefault?{...a,available_hours:newHours}:a)}));

            ...
            return(
              <div>
                ...
                {people.filter(p=>p.avail_h>0&&pTotals[p.id]>p.avail_h).map((p,i)=>(
                  <div key={i} style={{...glass,padding:"10px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:10,borderColor:"rgba(255,71,87,0.35)",background:"rgba(255,71,87,0.1)"}}>
                    <i className="ti ti-alert-triangle" style={{fontSize:16,color:"#FF4757",flexShrink:0}}/>
                    <span style={{fontSize:12,color:"#fff"}}><strong>{p.name}</strong> al {pct(pTotals[p.id],p.avail_h)}% — sobrecarga de <strong>{fmt(pTotals[p.id]-p.avail_h)} HH</strong></span>
                  </div>
                ))}
                {/* Charts */}
